 ...
 * @return string
 */
function AsposePdfImportExportEnqueueScripts() {
...
add_action('init', 'AsposePdfImportExportEnqueueScripts');

// For adding button for Aspose Cloud Pdf Importer
add_action('media_buttons_context',  'add_aspose_pdf_importer_exporter_button');

function add_aspose_pdf_importer_exporter_button($context){
    //path to my icon
...
add_action( 'admin_footer',  'aspose_pdf_import_export_add_inline_popup_content' );
function aspose_pdf_import_export_add_inline_popup_content() {
    ?>
	<style type="text/css">
        .ui-widget-overlay {
    ...
    <div id="aspose_pdf_popup_container" title="Aspose File Importer">
        ...
        <?php
            ...
            <div id="tabs">
                ...
                <div id="tabs-1">
                    <table>
                        ...
                        <tr>
                            ...
                            <td style="margin-left:10px; vertical-align: top;"> <input type="button" class="button-primary insert_content" content_type = "text" id="insert_pdf_content" value="Insert Text to Editor" />
                            <br /> <br />
                                <input type="button" class="button-primary insert_content" content_type = "html" id="insert_pdf_content" value="Insert HTML to Editor" />
                            </td>
                        </tr>
                ...
                <div id="tabs-2">
                    <input type="button" class="button-primary insert_content" content_type = "text" style="position:fixed; margin-top:5px; margin-left:350px;" id="insert_aspose_pdf_content" value="Insert Text to Editor" />
                    <br /> <br />
                    <input type="button" class="button-primary insert_content" content_type = "html" style="position:fixed; margin-top:35px; margin-left:350px;" id="insert_aspose_pdf_content" value="Insert HTML to Editor" />
...
add_filter('upload_mimes', 'aspose_pdf_importer_exporter_upload_mimes');

if (check_upload_aspose_pdf_import_export_context('Aspose-Pdf-Importer-Select-File')) {

    add_filter('media_upload_tabs', 'aspose_pdf_importer_exporter_image_tabs', 10, 1);
    add_filter('attachment_fields_to_edit', 'aspose_pdf_importer_exporter_action_button', 20, 2);
    add_filter('media_send_to_editor', 'aspose_pdf_importer_exporter_file_selected', 10, 3);
}

function aspose_pdf_importer_exporter_image_tabs($_default_tabs) {

    unset($_default_tabs['type_url']);
    return($_default_tabs);
}

function aspose_pdf_importer_exporter_upload_mimes ( $existing_mimes=array() ) {
...
function aspose_pdf_importer_exporter_action_button($form_fields, $post) {
...
function aspose_pdf_importer_exporter_file_selected($html, $send_id) {
...
function add_aspose_pdf_import_export_context_to_url($url, $type) {
...
function check_upload_aspose_pdf_import_export_context($context) {
    if (isset($_REQUEST['context']) && $_REQUEST['context'] == $context) {
        add_filter('media_upload_form_url', 'add_aspose_pdf_import_export_context_to_url', 10, 2);
        return TRUE;
    }
    return FALSE;
}
